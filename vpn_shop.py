...
async def generate_vless_key(user_id: int, days: int = 1, device: str = "phone") -> dict:
    """Генерация VLESS ключа для пользователя"""
    start = time.time()
    # Username только a-z, 0-9, underscore (3-32 символа)
    device_map = {"Телефон": "phone", "Ноутбук": "laptop", "Планшет": "tablet", "Тест": "trial"}
    device_en = device_map.get(device, device.lower().replace(" ", "_"))
    username = f"vpn_{user_id}_{int(time.time())}_{device_en}"
...
async def send_banner(event, banner_name: str, caption: str, buttons=None):
    """Отправляет баннер с картинкой"""
    start = time.time()
    user_id = event.sender_id if hasattr(event, 'sender_id') else event.chat_id
    
    logger.info(f"📤 Отправка баннера '{banner_name}' пользователю {user_id}")
    
    try:
        image_path = BANNERS.get(banner_name)
        if image_path and os.path.exists(image_path):
            with open(image_path, 'rb') as f:
                image_data = f.read()
            
            await event.respond(caption, file=image_data, buttons=buttons)
        else:
            logger.warning(f"⚠️ Баннер {image_path} не найден")
            await event.respond(caption, buttons=buttons)
        
        log_timing(f"Отправка баннера '{banner_name}'", start)
        logger.info(f"✅ Баннер '{banner_name}' отправлен")
    except Exception as e:
...
💬 Пиши мне — я отвечу!
    ...
    # Приветствие — без баннера (настроено через BotFather)
    await event.respond(
        "🚀 **KARINA VPN**\n\n"
        "Жмите кнопку **🚀 НАЧАТЬ** для продолжения 👇",
