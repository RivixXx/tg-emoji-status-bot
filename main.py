...
async def run_bot_main():
    ...
    async def vpn_stranger_interceptor(event):
        ...
        # ШАГ 4: Главное меню (Пользователь зарегистрирован)
            ...
            # Не блокируем событие, чтобы обычные скиллы тоже работали
            # Проверяем только текстовые команды меню
            if text in ["👤 Профиль", "🛒 Тарифы (Магазин)", "📖 Инструкция (FAQ)", "💳 Баланс", "👥 Рефералы", "🆘 Поддержка"]:
                logger.info(f"📱 Menu button pressed: {text}")
                ...
                if text == "👤 Профиль":
                    ...
                    orders = await mcp_vpn_get_user_orders(user_id, limit=5)
                    active_orders = len([o for o in orders if o.get("status") == "completed"])
                    ...
                    profile_text = (
                        ...
                        f"🆔 ID: `{user_id}`\n"
                        f"💳 Баланс: {balance} ₽\n"
                        f"⏳ Активных подписок: {active_orders}\n\n"
                    ...
                    referral_text = (
                        ...
                        f"🔗 **Твоя ссылка:**\n"
                        f"`{referral_link}`\n\n"
                        f"Просто отправь ссылку другу. Когда он купит подписку — ты получишь 10% на баланс!"
                    )
                    ...
                    support_text = (
                        ...
                        "• Telegram: @support\n"
                        "• Email: support@karina-ai.com\n\n"
                        "**Частые вопросы:**\n"
                        "• Ключ не работает — проверьте дату/время на устройстве\n"
                        "• Низкая скорость — попробуйте другой сервер\n"
                        "• Не начислилась комиссия — напишите в поддержку\n\n"
                        "⏰ Время работы поддержки: 9:00 - 21:00 МСК"
                    )
                    await event.respond(support_text)
                
                # Блокируем только если это кнопка меню
                raise events.StopPropagation
            # Если это не кнопка меню — не блокируем, пусть обрабатывают обычные скиллы
