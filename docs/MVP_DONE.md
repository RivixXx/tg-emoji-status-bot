...
- **[NEW] Самообучение**: Карина может сама решить сохранить факт из диалога с помощью инструмента `save_to_memory`.
...
- **[NEW] Защита от дублей**: Напоминания помечаются в БД как неактивные сразу после отправки.
- **[NEW] Защита от спама**: Бот блокирует повторные попытки смены эмодзи при ошибках до смены состояния.
...
- **[NEW] Реорганизация**: Все вспомогательные файлы и MD перенесены в `/docs`.
